...
_menuLinks.innerHTML = [
  '<a class="show-css-paths-anchor">Show CSS Paths</a>&nbsp;',
  '<a class="show-with-find-anchor">Show With Find</a>'
].join('');
...
_menuLinks.querySelector('.show-css-paths-anchor').addEventListener('click', togglePathInput);
...
function toggleFullPanel(show) {
  togglePathInput(show);
  mainInputElements.classList.toggle(show);
  if (show) {
    togglePanel('show', true, true);
  }
}
...
module.exports._menuLinks = _menuLinks;
...
module.exports.toggleFullPanel = toggleFullPanel;
