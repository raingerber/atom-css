...
function getMatchData(match) {
  ...
  var matchSelections = match.matches.map(function(_match) {
    ...
    var path = match.file.inputPath;
    console.log(_match.range);
    var fSel = normalizeTextRange(_match.range[0][0], match.file.map.rawCSS, path); // this should return FileSelection
    console.log(fSel);
    return fSel;
  });
