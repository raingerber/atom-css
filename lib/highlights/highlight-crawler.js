...
function getMatchData(match) {
  ...
  var rangeData = match.matches.map(function(_match) {
    var line = Point.sourceLineToAtomLine(_match.line);
    var text = arrayOfLines[line];
    var data = normalizeTextRange(_match.line, text);
  ...
  // return new FileSelection(match.file.inputPath, rangeData.range, rangeData.text);
  return rangeData;
}
...
function HighlightCrawler(matches, callback) {
  ...
  this.matches = matches.map(function(match) {
    ...
    return {
      file: match.file,
      // matches: matches.map(getMatchData)
