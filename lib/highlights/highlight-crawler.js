...
function FileSelection(path, range, text) {
  this.path = path;
  this.range = range;
  this.text = text;
}
...
function getMatchData(match) {
  console.log(match);
  // console.log(match.file.map.css);
  var arrayOfLines = match.file.map.css.match(/[^\r\n]+/g);
  // console.log(match.file.map.css);
  // console.log(line);
  // console.log(arrayOfLines[line]);

  // var text = editor.lineTextForBufferRow(line) || '';
  // console.error('params:', editor.getPath());
  var rangeData = match.matches.map(function(match) {
    var text = arrayOfLines[match.line - 1];
    console.log(match);
    console.log(text);
    return normalizeTextRange(match.line, text);
  });
  ...
  // console.error(rangeData);
  return new FileSelection(editor.getPath(), rangeData.range, rangeData.text);
  // return rangeData;
}
