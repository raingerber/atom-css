...
emitter.on('search', function(data) {
  ...
  window.setTimeout(function() {
    console.error('seassds');
    window.x = View._selectorInput.element;
    View._selectorInput.element.querySelector('.text-input').focus();
    console.log(View._selectorInput.element);
  }, 250);
...
emitter.on('match-clicked', function(match) {
  ...
  // console.log(match);
  crawler.setCurrentMatch(match.path, match.match.range.start);
});
